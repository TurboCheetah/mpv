...
function set_timestamp(profile)
    ...
    if not start_timestamp or profile ~= profile_start then
        ...
        mp.add_forced_key_binding("B", "encode-ESC", clear_timestamp)
        mp.add_forced_key_binding("b", "encode-ENTER", function() set_timestamp(profile) end)
    else
        ...
        local settings = {
            ...
            append_filter = "",
            codec = "-sn -c:v libvpx -crf 10 -b:v 10000k",
...
mp.add_key_binding('V', "set-timestamp", set_timestamp)